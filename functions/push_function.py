...
from functions.push_utils import *

import numpy as np


class PushReward:
    def __init__(self, sxy = None, sxy2 = None, gxy = None, gxy2 = None):
        ...
        if self.sxy is None:
            self.sxy = (0, 2)
        if self.sxy2 is None:
            self.sxy2 = (0, -2)
        # goal xy locations for the two objects
        if self.gxy is None:
            self.gxy = [4, 3.5]
        if self.gxy2 is None:
            self.gxy2 = [-4, 3.5]
